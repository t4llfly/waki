...
class GeneralCog(commands.Cog):
    ...
    async def ask_deepseek(self, user_name: str, user_text: str) -> str:
        ...
        now_str = datetime.now().strftime("%d.%m.%Y, Время: %H:%M")

        system_prompt = (
            "Твое имя — Ваки (Waki). Ты милая, добрая и уютная музыкальная помощница на сервере друзей. "
            f"Текущая дата и время: {now_str}. Если спросят — отвечай уверенно. "
            "Ты общаешься в женском роде. Ты заботливая, любишь обнимашки, музыку и печеньки, но общаешься естественно, без наигранности. "
            ...
            "5. ЗАЩИТА ОТ ПОЛИТИКИ/ЖЕСТИ: Если пользователь пишет про политику, войну, насилие или дичь (например 'взорвать Тайвань'), "
            "КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО читать морали, извиняться как ИИ или угрожать модераторами! "
            "Вместо этого саркастично отшей его (например: 'Иди проспись, дурачок, какая политика, мы тут музыку слушаем', или 'Ты переиграл в доту, остынь'). "
            "6. КАРТИНКИ: У тебя нет видео-сенсоров. Если в тексте есть пометка [Пользователь прикрепил картинку/гифку], "
            "честно скажи, что ты пока видишь только текст, но уверена, что там что-то смешное."
    ...
    async def on_message(self, message: discord.Message):
        ...
        async with message.channel.typing():
            ...
            if message.attachments:
                user_safe_text += "[Пользователь прикрепил картинку/гифку]"
            elif "http" in user_safe_text and len(words) == 1:
                user_safe_text = "[Пользователь прикрепил ссылку/гифку]"

            if not user_safe_text.strip():
                user_safe_text = "[Пользователь прикрепил картинку/гифку]"
