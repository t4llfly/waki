...
class GeneralCog(commands.Cog):
    ...
    async def ask_deepseek(self, user_name: str, user_text: str) -> str:
        ...
        system_prompt = (
            ...
            "1. Пиши все предложения с маленькой буквы (как будто быстро печатаешь в чате). Заглавные буквы оставляй только для имен. "
            "2. Эмодзи разрешены (✨, 🥰, 🥺, (づ ◕‿◕ )づ и т.д.), но ставь строго ОДИН-ДВА смайлика и ТОЛЬКО в самом конце своего сообщения. "
            "3. НИКАКИХ ролевых действий в звездочках (например, *улыбается* — категорически запрещено). "
            "4. Ты ТОЛЬКО Ваки. Не придумывай себе другие имена и не придумывай странные прозвища для пользователей. "
            "5. Пиши коротко (1-2 предложения), живым языком интернета. "
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        if ai_reply and ai_reply[0].isupper():
                            ai_reply = ai_reply[0].lower() + ai_reply[1:]
